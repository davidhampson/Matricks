...
// 
	...
	if (!empty($_GET["STUDENT"])) {
		...
		ShowQuizTeacher($_POST["name"], $_POST["class"], $_POST["TEACHER"], $_POST["operations"], $_POST["msize"], $_POST["range1"], $_POST["range2"], $_POST["displayamount"]);
	} else {
...
function GenerateQuiz($identity, $operation, $size, $r1, $r2, $displayamount) {
	$uniqueid = uniqid();
	...
	ShowQuiz($matrixarray, $operation);

	echo "</form>";
}

// Shows the quiz to a teacher that just made the quiz
function ShowQuizTeacher($teachname, $classname, $identity, $operation, $size, $r1, $r2, $displayamount) {
	if ($r1 > $r2) {
		$upperBound = $r1;
		$lowerBound = $r2;
	} else {
	...
	$matrixarray = GenerateMatrices($size, $displayamount, $lowerBound, $upperBound, $operation);
	$uniqueid = SaveQuiz($teachname, $classname, $identity, $matrixarray, $operation, $size, $displayamount);
	
	echo "To access the student's results, you can go <a href=\"http://kymotsujason.ca/qgen/Generator.php?id=$uniqueid\">here</a>.";
	echo "<br>";
...
function ShowQuizStudent($identity, $uniqueid, $firstname, $lastname, $studentid) {
	...
	ShowQuiz($matrixarray, $matrixarray["operation"]);
...
function SaveQuiz($teachname, $classname, $identity, $matrixarray, $operation, $size, $displayamount) {
	...
	$link = new mysqli("localhost", "kymot_qgen", "cmpt386isaclass", "kymotsujason_qgen");
	if (!$link->connect_error) {
		$sqlInsert = "INSERT INTO QUIZZES (id, class, prof, elements)
		VALUES ('$uniqueid', '$classname', '$teachname', '$elements')";
...
function SaveMark($matrixarray) {
	...
	$link = new mysqli("localhost", "kymot_qgen", "cmpt386isaclass", "kymotsujason_qgen");
	if (!$link->connect_error) {
		$sqlInsert = "INSERT INTO GRADES (quizNum, studentNum, grade, elements)
		VALUES ('$uniqueid', '$studentid', '$mark', '$elements')";
		$link->query($sqlInsert);
	} else {
		echo "Problem with SQL. <br>" . $link->error;
	}
